...
class Auro(commands.AutoShardedBot):
            ...
            for filename in os.listdir(folder):
                if filename.endswith(".py") and not filename.startswith("__"):
                    ...
                    try:
                        ...
                        print(f"{self.get_time()} | SUCCESS   | Module Loaded: {folder_name}.{cog_name}")
                    except Exception as e:
                        print(f"{self.get_time()} | ERROR     | Failed to load {cog_name}: {e}")
    ...
    async def on_ready(self):
        await self.change_presence(
            ...
            activity=discord.Activity(type=discord.ActivityType.custom, name="Auro", state="🍃")
        )
