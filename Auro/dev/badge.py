...
class Badges(commands.Cog):
    ...
    @commands.is_owner()
    async def add_badge_command(self, ctx: commands.Context, User: discord.User = None, badge: str = None):
        """Add a Badge to a User"""
        if User is None or badge is None:
            await ctx.reply(f"Missing required arguments. Usage: `addbadge <user> <badge>`",delete_after=3)
            return True
    ...
    async def show_badge(self, ctx: commands.Context, User: discord.User = None):
        ...
        ).set_thumbnail(url=User.avatar.url if User.avatar else None).set_footer(text=f"Total Badges: {len(badges)}")
        await ctx.reply(embed=badges_embed,delete_after=30)
        return True
    
    @commands.command(name="delbadges",aliases=["removebadge","removebadges","delbadge"])
    @commands.is_owner()
    async def del_badges(self, ctx: commands.Context, User: discord.User = None, badge: str = None):
        
        if User is None or badge is None:
            await ctx.reply(f"> Missing required arguments. Usage: `delbadges <user> <badge>`",delete_after=3)
            return True
        ...
        try:
            ...
            await ctx.reply(f" {_Emojis.success} Removed badge {badge} from {User.name}",delete_after=5)
        except Exception as e:
