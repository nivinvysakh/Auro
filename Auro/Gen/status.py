...
class Stats(commands.Cog):
    ...
    @commands.hybrid_command(name="stats", description="📊 System and Lavalink Dashboard")
    async def stats(self, ctx: commands.Context):
        await ctx.defer()
        
        uptime = str(time.strftime("%Hh %Mm %Ss", time.gmtime(time.time() - self.start_time)))
        ...
        bot_embed.add_field(name="🛰️ Latency", value=f"`{round(self.bot.latency * 1000)}ms`", inline=True)
        bot_embed.add_field(name="⏳ Uptime", value=f"`{uptime}`", inline=True)
        bot_embed.add_field(name="🧠 Memory", value=f"`{memory_usage:.2f} MB`", inline=True)
        bot_embed.add_field(name="⚙️ Environment", value=f"`{platform.system()}` | `Py {platform.python_version()}`", inline=False)
        bot_embed.set_footer(text="Auro System Layer")
        bot_embed.set_thumbnail(url=self.bot.user.display_avatar.url)

        lava_embed = discord.Embed(
            title="🎵 Lavalink Media Engine",
            color=0x5865F2
        )
