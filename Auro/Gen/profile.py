...
class ProfileButtons(discord.ui.View):
    ...
    async def apply_tiered_theme(self, embed: discord.Embed, page_type: str):
        ...
        if is_owner:
            ...
            embed.title = f"{self.target.name}'s {page_type.title()} [{BadgesIcon.developer}]"
            embed.set_author(name="eco.plse",icon_url="https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcQeX-ZBv4hBS5RnULagS2YA7gMBgb0p_lCc9g&s")
            gifs = {
    ...
    async def interaction_check(self, interaction: discord.Interaction) -> bool:
        if interaction.user.id != self.command_owner.id:
            await interaction.response.send_message(f"{emojis.error} Only {self.command_owner.name} can interact!", ephemeral=True)
            return False
        return True

    async def handle_button_state(self, interaction: discord.Interaction, clicked_button: discord.ui.Button):
    ...
    def get_member_devices(self, member):
        if member.web_status != discord.Status.offline: return "Web"
        if member.desktop_status != discord.Status.offline: return "Desktop"
        if member.mobile_status != discord.Status.offline: return "Mobile"
        return "N/A"
    
    @discord.ui.button(label="Status", style=discord.ButtonStyle.secondary, emoji=ButtonEmojis.status)
    async def status_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        await self.handle_button_state(interaction, button)
        member = interaction.guild.get_member(self.target.id)
        
        embed = discord.Embed()
        embed.add_field(name="Status", value=self.memberstatemoji(member) if member else "Offline", inline=True)
        embed.add_field(name=f"{emojis.devicesicon} Device" , value=f"`{self.get_member_devices(member) if member else 'N/A'}`", inline=True)
    ...
    @discord.ui.button(label="Account", style=discord.ButtonStyle.secondary, emoji=ButtonEmojis.account)
    async def account_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed.add_field(name=f"{emojis.time} Created", value=f"<t:{int(self.target.created_at.timestamp())}:R>", inline=True)
        embed.add_field(name=f"{emojis.join} Joined", value=f"<t:{int(member.joined_at.timestamp())}:R>" if member else "N/A", inline=True)
        embed.add_field(name=f"{emojis.mutual} Mutuals", value=f"`{len(self.target.mutual_guilds)}`", inline=True)
        embed.add_field(name=f"{emojis.id} ID", value=f"`{self.target.id}`", inline=True)

        if member:
            embed.add_field(name=f"{emojis.boost} Booster", value=f"{emojis.success}" if member.premium_since else f"{emojis.error}", inline=True)
            embed.add_field(name=f"{emojis.roles} Roles", value=f"`{len(member.roles) - 1}`", inline=True)
        
        await self.apply_tiered_theme(embed, "account")
        await interaction.edit_original_response(embed=embed, view=self)

    @discord.ui.button(label="Badges", style=discord.ButtonStyle.secondary, emoji=ButtonEmojis.badges)
    async def badges_button(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        embed = discord.Embed().set_thumbnail(url=self.target.display_avatar.url)
        if badges:
            display = "\n".join([f"{emojis.dot} **{b.replace('_', ' ')}** | {self.badge_mapping.get(b, '🏅')}" for b in badges])
...
class Profile(commands.Cog):
    ...
    @commands.hybrid_command(name="profile", description="View a user profile", aliases=["pr"])
    async def profile(self, ctx: commands.Context, user: discord.User = None):
        ...
        embed.add_field(name=f"{emojis.id} ID", value=f"`{target_user.id}`", inline=True)
        embed.add_field(name=f"{emojis.heart} Name", value=f"*{target_user.global_name or target_user.name}*", inline=True)
        
        member = ctx.guild.get_member(target_user.id)
        nickname = f"*{member.nick}*" if member and member.nick else f"{emojis.error} None"
        embed.add_field(name=f"{emojis.nickname_emoji} Nickname", value=nickname, inline=True)
