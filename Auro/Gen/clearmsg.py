...
class PurgeBot(commands.Cog):
    ...
    @commands.hybrid_command(name="clearbot", description="🧹 Clean up all bot responses in the channel!")
    @commands.guild_only()
    ...
    async def clearbot(self, ctx: commands.Context):
        ...
        history = [m async for m in ctx.channel.history(limit=50) if m.author == self.bot.user]
        
        if not history:
            
            return await ctx.interaction.edit_original_response(content=f"{emojis.error} No bot messages found in the last 50 messages.")

        
        deleted = await ctx.channel.purge(limit=50, check=lambda m: m.author == self.bot.user)
    ...
    @commands.hybrid_command(name="clearmsg", description="🧹 Clean up messages in the channel!")
    @commands.guild_only()
    @commands.has_permissions(manage_messages=True)
