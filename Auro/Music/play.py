...
class Music(commands.Cog):
    ...
    async def on_pomice_track_start(self, player: Player, track: pomice.Track):
        ...
        if isinstance(player.channel, discord.StageChannel):
            try:
                ...
                if player.controller:
                    await player.controller.send(f"{Emojis.warning} I need speaker permissions!")
        thumbnail = getattr(track, "thumbnail", None) or track.info.get("thumbnail") or track.info.get("artworkUrl")
        ...
        if player.controller:
            ...
            ).set_thumbnail(url=thumbnail).set_footer(
    ...
    @commands.hybrid_command(name="play", aliases=["p"],description="🎶 Play a song from YouTube / SoundCloud / Spotify ")
    @commands.guild_only()
    ...
    async def play(self, ctx: commands.Context, *, search: str):
        ...
        if isinstance(results, pomice.Playlist):
            ...
            if added == 0:
                return await ctx.send(f"{Emojis.warning} No suitable tracks found (Filtered Live/Documentaries).")

            if not player.is_playing:
                await player.do_next()

            return await ctx.send(f"{Emojis.success} Loaded **{added}** tracks from playlist.")
        ...
        if not valid_track:
            return await ctx.send(f"{Emojis.warning} That track was filtered (Live/Too long/Too short).")
    ...
    @commands.hybrid_command(name="skip",description="⏭️ Skips the current song and plays the next one.")
    async def skip(self, ctx):
        player = cast(Player, ctx.voice_client)
        if not player or not player.is_playing:
            embed  = discord.Embed(
                title=f"{Emojis.warning} Nothing Playing",
                color= discord.Color.yellow()
            ).set_footer(text="Auro Engine • Warning",icon_url=self.bot.user.display_avatar.url)
    ...
    @commands.hybrid_command(name="stop",description=" ❌ Stops the music, clears the queue, and leaves.")
    async def stop(self, ctx):
        if ctx.voice_client:
            player = cast(Player, ctx.voice_client)
            try: await player.channel.edit(status=None)
            except: pass
            ...
            embed = discord.Embed(
                ...
                color= discord.Color.blurple()
            ).set_footer(text="Auro Engine • Offline",icon_url=self.bot.user.display_avatar.url)
            await ctx.reply(embed=embed)

    @commands.hybrid_command(name="queue",description="🎶 Shows the current song and upcoming tracks.")
    async def queue(self, ctx):
        ...
        await ctx.reply(embed=embed)
    @commands.hybrid_command(name="loop",description="🔁 Toggles looping for the current song.",aliases=["ls","repeat"])
    @commands.guild_only()
    async def loop(self,ctx:commands.Context):
        ...
        embed.set_footer(text="Auro Engine • Loop Logic",icon_url=self.bot.user.display_avatar.url)

        await ctx.reply(embed=embed)

    @commands.hybrid_command(name="loopqueue", aliases=["lq"], description="🔁 Toggles looping for the entire queue.")
    @commands.guild_only()
    async def loop_queue(self,ctx:commands.Context):
        ...
        ).set_footer(text="Auro Engine • Queue System",icon_url=self.bot.user.display_avatar.url)
        await ctx.reply(embed=embed)
async def setup(bot):
