...
class LyricsView(discord.ui.View):
    ...
    @discord.ui.button(label="Back", style=discord.ButtonStyle.gray, emoji=Emojis.left_arrow)
    ...
    @discord.ui.button(label="Next", style=discord.ButtonStyle.gray, emoji=Emojis.right_arrow)
...
class Lyrics(commands.Cog):
    ...
    @commands.hybrid_command(name="lyrics", description="Get the lyrics of the currently playing song")
    @commands.guild_only()
    async def lyrics(self, ctx: commands.Context):
        ...
        if not lrc_data:
            return await ctx.send(f"{Emojis.error} No synced lyrics found for **{track.title}**.")
        ...
        pages = ["\n".join(processed_lines[i:i+10]) for i in range(0, len(processed_lines), 10)]
    ...
    @commands.hybrid_command(name="seek", description="Jump to a specific time in the song")
    @commands.guild_only()
    ...
    async def seek(self, ctx: commands.Context, time: str):
        ...
        if seek_ms < 0:
            return await ctx.reply(f"{Emojis.error} Invalid format. Use `mm:ss` or `hh:mm:ss`.")
        if seek_ms > player.current.length:
            return await ctx.reply(f"{Emojis.error} Time exceeds track length.")
        
        await player.seek(seek_ms)
        await ctx.reply(f"{Emojis.success} Seeked to {self.format_time(seek_ms)}.", delete_after=5)
