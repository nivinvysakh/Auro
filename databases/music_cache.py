...
class MusicCache:
    ...
    async def clear_guild_cache(self, guild_id: int):
        ...
        async with aiosqlite.connect(self.path) as db:
            await db.execute("PRAGMA synchronous=NORMAL;")
            await db.execute(
                "DELETE FROM music_cache WHERE query = ?", (f"loop_{guild_id}".lower(),)
            )
            await db.commit()
    ...
    async def clear_guild_cache_by_query(self, query: str):
        ...
        async with aiosqlite.connect(self.path) as db:
            await db.execute("PRAGMA synchronous=NORMAL;")
            await db.execute(
                "DELETE FROM music_cache WHERE query = ?", (query.strip().lower(),)
            )
            await db.commit()
    ...
    async def clear_all_guild_cache(self, guild_id: int):
        ...
        async with aiosqlite.connect(self.path) as db:
            await db.execute("PRAGMA synchronous=NORMAL;")
            await db.execute(
                "DELETE FROM music_cache WHERE query LIKE ?", (f"%{guild_id}%",)
            )
            await db.commit()
    ...
    async def clear_loop_queue(self, guild_id: int):
        async with aiosqlite.connect(self.path) as db:
            ...
            await db.execute(
                "DELETE FROM music_cache WHERE query LIKE ?", (query_pattern,)
            )
            await db.commit()
    ...
    async def get_all(self):
        ...
        async with aiosqlite.connect(self.path) as db:
            await db.execute("PRAGMA synchronous=NORMAL;")
            async with db.execute(
                "SELECT query, track_hash, title FROM music_cache ORDER BY query"
            ) as cursor:
